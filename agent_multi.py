...
def _build_orchestrator(llm, checkpointer=None):
    ORQ_SYSTEM_PROMPT = (
        ...
        "   b) 'produção de hoje' / 'do dia' → use ctx.today():\n"
        "      hoje = ctx.today()\n"
        "      ctx.api(f'/production?from={hoje}&to={hoje}')\n"
        "   c) 'últimos N dias' → use ctx.date_range(days=N):\n"
        "      from_date, to_date = ctx.date_range(days=7)\n"
        "      ctx.api(f'/production?from={from_date}&to={to_date}')\n"
        "   d) 'período da tarefa' (segue a frequência) → use os parâmetros recebidos:\n"
        "      ctx.api(f'/production?from={from_date}&to={to_date}')\n"
        ...
        "   i) MONITORES DE THRESHOLD: se a tarefa monitora um valor e chama ctx.notify_alert(),\n"
        "      o run() DEVE sempre retornar o valor calculado como string — mesmo quando o\n"
        "      threshold não for cruzado. Isso valida que o cálculo funciona no test_task_code.\n"
        "      Exemplo obrigatório para monitores:\n"
        "        oee = df['oee_a'].mean()\n"
        "        if oee < 80: ctx.notify_alert(f'OEE {oee:.1f}% abaixo de 80%', value=oee, threshold=80)\n"
        "        return f'OEE médio: {oee:.2f}% (threshold: 80%)'\n"
        ...
        "6. Responda ao usuário EXATAMENTE neste formato, sem adicionar mais nada:\n"
